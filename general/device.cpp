...
Device::Device()
{
   if (std::getenv("MFEM_MEMORY") && !mem_host_env && !mem_device_env)
   {
      std::string mem_backend(std::getenv("MFEM_MEMORY"));
      if (mem_backend == "host")
      {
         mem_host_env = true;
   ...
   if (std::getenv("MFEM_DEVICE"))
   {
      std::string device(std::getenv("MFEM_DEVICE"));
      Configure(device);
      device_env = true;
   }
}
