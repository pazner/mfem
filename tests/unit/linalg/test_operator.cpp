...
TEST_CASE("Sum and product operators", "[Operator]")
{
   ...
   auto op_2 = Handle<IdentityOperator>::MakeOwning(n);

   // op_1 will not be owned, op_2 will be owned
   SumOperator sum(&op_1, 1.0, op_2, 1.0);
   ProductOperator product(&op_1, op_2);
   // Note: it is not a problem for triple to own op_2 'twice'
   TripleProductOperator triple(&op_1, op_2, op_2);
